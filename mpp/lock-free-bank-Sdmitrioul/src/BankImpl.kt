...
class BankImpl(override val numberOfAccounts: Int) : Bank {
    ...
    private inner class TransferOp(val fromIndex: Int, val toIndex: Int, val amount: Long) : Op() {
        ...
        override fun invokeOperation() {
            ...
            val smallest = acquire(minIndex, this)
            val biggest = acquire(maxIndex, this)

            transfer(maxIndex, biggest, smallest)

            release(minIndex, this)
            release(maxIndex, this)
        }

        private fun transfer(maxIndex: Int, biggest: AcquiredAccount?, smallest: AcquiredAccount?) {
            biggest ?: return
            smallest ?: return

            val from = if (maxIndex == fromIndex) biggest else smallest
            val to = if (maxIndex == toIndex) biggest else smallest
