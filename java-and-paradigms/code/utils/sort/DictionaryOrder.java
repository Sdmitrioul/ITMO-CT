...
public class DictionaryOrder implements Sorting {
    private ArrayList<Map.Entry<String, String>> list;
    private Map<String, String> map;

    public DictionaryOrder(ArrayList<String> list) {
        map = new LinkedHashMap<>();
        for (String iterator : list) {
            StringBuilder stringBuilder = new StringBuilder();
            for (int i = 0; i < iterator.length(); i++) {
                if (Character.isWhitespace(iterator.charAt(i)) || Character.isDigit(iterator.charAt(i)) || Character.isLetter(iterator.charAt(i))) {
