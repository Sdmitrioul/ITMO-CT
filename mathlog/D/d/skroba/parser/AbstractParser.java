package d.skroba.parser;

import d.skroba.parser.source.CharSource;
...
public abstract class AbstractParser<T> implements Parser<T>  {
	...
	/**
	 * Assign prev char from source to ch, if source hasn't another chars give to it {@code '0xffff'}.
	 */
	protected void prevChar() {
		ch = source.hasPrev() ? source.prev() : START;
	}
	
	protected void currentChar() {
		ch = source.current();
	}
	...
	protected boolean backTest(final char expected) {
		if (ch == expected) {
			prevChar();
			return true;
		}
